...
namespace LevelExtender
{
    ...
    internal class Commands
    {
        ...
        public Commands(ModEntry modEntry, ModConfig config, IMonitor? monitor = null)
        {
            this._modEntry = modEntry;
            this._config = config;
            this._monitor = monitor ?? modEntry.Monitor;
        }

        public void ShowExperienceSummary(string command, string[] args)
        {
            this._monitor?.Log("Skill      | Level | Current XP   | XP for Next Lvl", LogLevel.Info);
            this._monitor?.Log("----------------------------------------------------", LogLevel.Info);

            foreach (var skill in this._modEntry.Skills)
            {
                int xpForNextLevel = skill.GetRequiredExperienceForLevel(skill.Level);
                this._monitor?.Log($"{skill.Name,-10} | {skill.Level,-5} | {skill.Experience,-12} | {xpForNextLevel}", LogLevel.Info);
            }
        }
        ...
        public void SetLevel(string command, string[] args)
        {
            ...
            {
                this._monitor?.Log("Command failed. Usage: le_setlevel <skillname> <level>", LogLevel.Error);
                return;
            }
            ...
            {
                skill.Level = level;
                this._monitor?.Log($"{skill.Name} level set to {level}. XP adjusted accordingly.", LogLevel.Info);
            }
        }
        ...
        public void SetExperience(string command, string[] args)
        {
            ...
            {
                this._monitor?.Log("Command failed. Usage: le_setxp <skillname> <amount>", LogLevel.Error);
                return;
            }
            ...
            {
                skill.Experience = xp;
                this._monitor?.Log($"{skill.Name} XP set to {xp}. Level has been updated.", LogLevel.Info);
            }
        }
        ...
        public void ToggleWorldMonsters(string command, string[] args)
        {
            ...
            this._monitor?.Log($"Wilderness monster spawning for this save is now {(_config.EnableWorldMonsters ? "ON" : "OFF")}.", LogLevel.Info);
        }

        public void ToggleDrawBars(string command, string[] args)
        {
            _config.DrawXpBars = !_config.DrawXpBars;
            this._monitor?.Log($"XP bars will now be {(_config.DrawXpBars ? "drawn" : "hidden")}.", LogLevel.Info);
        }
        ...
        public void SetExperienceModifier(string command, string[] args)
        {
            ...
            {
                this._monitor?.Log("Command failed. Usage: le_xp_modifier <skillname> <modifier>.", LogLevel.Error);
                return;
            }
            ...
            if (skill != null)
            {
                ...
                this._monitor?.Log($"XP Modifier for {skill.Name} set to {modifier}.", LogLevel.Info);
            }
        }
        ...
        public void ShowSkillXpTable(string command, string[] args)
        {
            ...
            if (args.Length < 1)
            {
                this._monitor?.Log("Command failed. Usage: le_xp_table <skillname>.", LogLevel.Error);
                return;
            }
            ...
            if (skill != null)
            {
                ...
                this._monitor?.Log(sb.ToString(), LogLevel.Info);
            }
        }
        ...
        public void ToggleExtraItemNotifications(string command, string[] args)
        {
            ...
            this._monitor?.Log($"Extra item notifications will now be {(_config.DrawExtraItemNotifications ? "shown" : "hidden")}.", LogLevel.Info);
        }
        ...
        public void SetMinNotificationPrice(string command, string[] args)
        {
            ...
            {
                this._monitor?.Log("Command failed. Usage: le_min_notification_price <amount>.", LogLevel.Error);
                return;
            }

            _config.MinItemPriceForNotifications = price;
            this._monitor?.Log($"Minimum price for extra item notifications set to {price}g.", LogLevel.Info);
        }
        ...
        private Skill GetSkill(string name)
        {
            ...
            if (skill == null)
            {
                this._monitor?.Log($"Could not find a skill named '{name}'.", LogLevel.Error);
            }
            return skill;
        }
    }
